...
- `POST /agent` now requires explicit confirmation:
  ...
  - `/agent --confirm <intent-id>` confirms and applies transition
- `POST /batch --generate --unified` now supports branch governance parity:
  ...
  - may return governance intent requiring `--confirm <intent-id>` to create session branch
...
```json
POST /agent
{ "workspaceRoot": "C:\\repo", "confirmIntentId": "<intent-id>" }
```
...
```json
POST /review-auto
{ "workspaceRoot": "C:\\repo", "action": "orchestrate", "confirmIntentId": "<intent-id>" }
```
