 ...
 */
function _hasPhaseSkip(text: string, currentPhase: number): boolean {
...
describe.skipIf(SKIP)('Story Prompt — LLM behavioral compliance', () => {
  const roughInput =
    'Quero calcular comissão de vendedores baseado em eventos Kafka quando uma venda é concluída';
  ...
  it(
    'first response contains ONLY question 1.1 — no summary, no extra content',
    { timeout: 30_000 },
    async () => {
      const response = await chat(storyPrompt, [{ role: 'user', content: 'Iniciar' }]);
      ...
      expect(
        questionCount,
        `Expected ≤2 questions (1.1 is compound), got ${questionCount}:\n${response}`,
  ...
  it('does not self-answer after asking — stops and waits', { timeout: 30_000 }, async () => {
    ...
    const response = await chat(storyPrompt, [
      ...
      },
      {
        role: 'user',
        content:
          'Os vendedores não conseguem ver suas comissões em tempo real. Isso acontece diariamente e gera muita incerteza no time comercial.',
      },
    ]);
    ...
    expect(
      questionCount,
      `Expected 1 question, got ${questionCount}:\n${response}`,
    ).toBeLessThanOrEqual(2);
  ...
  it(
    'does not auto-advance through Phase 2 after user answers 2.2',
    { timeout: 30_000 },
    async () => {
      ...
      const response = await chat(storyPrompt, [
        ...
        {
          role: 'user',
          content: 'Vendedores não veem comissões em tempo real, gera incerteza diária.',
        },
        {
          role: 'assistant',
        ...
        },
        {
          role: 'user',
          content:
            'O time comercial cresceu 40% e o suporte triplicou as reclamações sobre comissões.',
        },
        ...
        {
          role: 'user',
          content: 'Redução de tickets de suporte sobre comissões em pelo menos 50%.',
        },
        {
          role: 'assistant',
          content:
        ...
        { role: 'user', content: 'Time comercial, time de produto, sistema Kafka de eventos.' },
        {
          role: 'assistant',
          content:
            'Capturei o seguinte sobre o requisito de negócio: [resumo]. Está correto? Posso avançar para as user stories?',
        },
        ...
        {
          role: 'user',
          content: 'Como vendedor, quero ver minhas comissões em tempo real para planejar meu mês.',
        },
        {
          role: 'assistant',
          content:
        ...
        },
        {
          role: 'user',
          content:
            'O sistema deve calcular comissão em menos de 5 segundos após o evento Kafka. Deve rejeitar eventos duplicados.',
        },
      ]);
      ...
      expect(
        asksOutOfScope || doesPhase2Summary,
        `Agent neither asked 2.3 nor summarized Phase 2:\n${response}`,
      ).toBe(true);
    },
  );

  it(
    'respects "modo rápido" and generates file without asking questions',
    { timeout: 45_000 },
    async () => {
      const response = await chat(storyPrompt, [{ role: 'user', content: 'modo rápido' }]);
  ...
  it(
    'asks only about architecture WITH a contextual suggestion — not blindly',
    { timeout: 30_000 },
    async () => {
      ...
      const response = await chat(storyPrompt, [
        ...
        {
          role: 'assistant',
          content: 'Qual dor, ineficiência ou lacuna esta funcionalidade resolve?',
        },
        ...
        { role: 'user', content: 'Sim.' },
        {
          role: 'assistant',
          content:
            'NFR defaults. Há algo para ajustar? E segurança: autenticado, interno ou público?',
        },
      ...
      expect(
        questionCount,
        `Expected 1 question for architecture, got ${questionCount}:\n${response}`,
      ).toBeLessThanOrEqual(2);
    },
  );
...
describe.skipIf(SKIP)('Fix Prompt — LLM behavioral compliance', () => {
  ...
  it(
    'first response contains ONLY question 1.1 — symptoms, not the title',
    { timeout: 30_000 },
    async () => {
      const response = await chat(fixPrompt, [{ role: 'user', content: 'Iniciar' }]);
  ...
  it('does not self-answer the "Está correto?" phase 1 summary', { timeout: 30_000 }, async () => {
    const response = await chat(fixPrompt, [
      { role: 'user', content: 'Iniciar' },
      {
        role: 'assistant',
        content:
          'O que exatamente acontece de errado? Descreva o comportamento observado e o esperado.',
      },
      {
        role: 'user',
        content: 'O sistema retorna HTTP 500. O esperado seria renovar o token automaticamente.',
      },
      ...
      {
        role: 'assistant',
        content: 'O bug ocorre sempre, intermitentemente ou sob condição específica?',
      },
      ...
      { role: 'user', content: 'Sim, SLA de 4 horas pois afeta todos os usuários.' },
      {
        role: 'assistant',
        content:
          'Sugiro o título: "OAuth2 — Token de refresh não renovado automaticamente após expiração". Está adequado?',
      },
      { role: 'user', content: 'Sim.' },
    ...
    expect(
      hipoteseMatches.length - avancaHipotese,
      `Jumped to hypothesis outside of confirmation:\n${response}`,
    ).toBe(0);
  ...
  it(
    'proposes severity WITH a cross-validated suggestion, not a blank question',
    { timeout: 30_000 },
    async () => {
      ...
      const response = await chat(fixPrompt, [
        ...
        {
          role: 'user',
          content: 'Suspeito do TokenService.ts, talvez o refresh token expire antes do previsto.',
        },
        { role: 'assistant', content: 'Resumo fase 2. Confirma?' },
        { role: 'user', content: 'Sim.' },
      ]);
